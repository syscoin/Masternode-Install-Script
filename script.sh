...
start_syscoind(){
  ...
  sudo systemctl enable syscoind
  clear
}
...
create_and_configure_syscoin_user(){
  ...
  grep -q "alias syscoind" ~/.bashrc || echo "alias syscoind='echo \"systemd is running as a service - please stop that process first, then use sysd instead of systemd\"'" >> ~/.bashrc
...
node_bls_key(){
  read -e -p "Sentry Node BLS Secret Key [$nodeblsprivkey]: " NODE_BLS_KEY
